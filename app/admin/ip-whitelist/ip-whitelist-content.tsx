...
export function IpWhitelistContent({ initialList, variant = 'page' }: IpWhitelistContentProps) {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await fetch(`/api/admin/ip-whitelist/${itemToDelete.id}`, {
                method: 'DELETE',
            });
            const result = await response.json();
    ...
    return (
        <div className="space-y-6">
            {variant === 'page' ? (
                <div>
                    <h2 className="text-3xl font-bold">IP 白名单</h2>
                    <p className="text-muted-foreground">
                        白名单中的 IP 可免密钥访问字体 API，且支持分钟级限流保护系统
                    </p>
                </div>
            ) : null}
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>仅支持单个 IP（IPv4/IPv6）</CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    {sortedList.length === 0 ? (
                        ...
                        <div className="overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className="min-w-[220px]">IP</TableHead>
            ...
            <Dialog open={editOpen} onOpenChange={setEditOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>编辑 IP 白名单</DialogTitle>
                        <DialogDescription>{itemToEdit?.ip}</DialogDescription>
                    </DialogHeader>
            ...
            <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>删除后该 IP 将恢复为常规访问策略。</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
