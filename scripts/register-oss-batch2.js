#!/usr/bin/env node
/**
 * Register OSS batch2 fonts into vault prod.db from oss-batch2-map.json.
 * Copy map + this script into container /app then:
 *   docker exec fonts-vault node /app/register-oss-batch2.js
...
const WEIGHT_NUM = {
  ...
  Black: 900,
  Heavy: 900,
  // ChillPixels style faces (not traditional weights)
  Mono: 400,
  Maximal: 500,
  Minimalism: 300,
  Matrix: 600,
...
(async () => {
  const mapPath = process.argv[2]
    ? process.argv[2]
    : fs.existsSync('/app/oss-batch2-map.json')
      ? '/app/oss-batch2-map.json'
      : 'oss-batch2-map.json';
    ...
    try {
      await c.execute({
        ...
        args: [
          ...
          JSON.stringify([]),
          JSON.stringify(fam.languages || ['简体中文', '繁体中文', '英语']),
          JSON.stringify(fam.useCases || ['正文', '标题', '屏幕阅读']),
